...
$.widget("ui.multiselect", {
  options: {
    ...
    this.selectedActions  = $('<div class="actions ui-widget-header ui-helper-clearfix"><input type="text" class="search-selected ui-widget-content ui-corner-all"/><span class="count">'+$.tmpl($.ui.multiselect.locale.itemsCount,{count:0})+'</span><a href="#" class="remove-all">'+$.tmpl($.ui.multiselect.locale.removeAll)+'</a></div>').appendTo(this.selectedContainer);    
		this.availableActions = $('<div class="actions ui-widget-header ui-helper-clearfix"><span class="busy">'+$.tmpl($.ui.multiselect.locale.busy)+'</span><input type="text" class="search ui-widget-content ui-corner-all"/><span class="count">'+$.ui.multiselect.locale.instruction+'</span><a href="#" class="add-all">'+$.tmpl($.ui.multiselect.locale.addAll)+'</a></div>').appendTo(this.availableContainer);
    ...
    this._registerSearchEvents(this.selectedContainer.find('input.search-selected'), true);
		...
		//this.availableActions.find('input').width(Math.max(this.availableActions.width() - this.availableActions.find('a.add-all').width() - 60, 20));
    //this.selectedActions.find('input').width(Math.max(this.selectedActions.width() - this.selectedActions.find('a.remove-all').width() - this.selectedActions.find('span.count').width() - 60, 20));
		...
		// init lists
    optionList  = that.element.find('option');
    optionCount = optionList.length; 
    
    if( (optionCount > 50) && this.options.dataLimiter){ 
      that._setBusy(true);
      setTimeout(function(){        
        that._loadOptions(optionList, 0);        
        that._updateCount();
        that._setBusy(false);
      }, 250);
    }
    else{
      this._populateLists(optionList);      
    }
	},
  ...
  _loadOptions: function(options, counter) {
    var that = this;
    
    //that._setBusy(true);        
    currentOption = options[0];
    
    var list = (currentOption.selected ? that.selectedList : that.availableList);
    var item = that._getOptionNode(currentOption).show();
    that._applyItemState(item, currentOption.selected);
    item.data('multiselect.idx', counter);
    ...
    {
      setTimeout(function(){ 
        that._loadOptions(options.next(), counter);
      }, 0);      
    }
	...
	_setData: function(key, value) {
        ...
        this._registerSearchEvents(this.selectedContainer.find('input.search-selected'), false);
				break;
		...
		if (opts[side].sortable) {
			list.sortable({
				...
				receive: function(event, ui) {
					// DEBUG
          //console.log("Receive : %s:%s = %s ",ui.item.data('multiselect.optionLink'), ui.item.parent()[0].className, itemSelected);
	...
	_getOptionNode: function(option) {
        ...
        // if type == 'group', hide email from the item
        if (this.options.type == 'group') {
            var email_string_pos = option.text().search(new RegExp(/ - /i));

            if (email_string_pos >= 0) {
                var option_text_without_email = option.text().substr(0, email_string_pos);
                var node = $('<li class="ui-state-default ui-element" title="'+option.text()+'"><span class="ui-icon"/>' + option_text_without_email + '<a href="#" class="action"><span class="ui-corner-all ui-icon"/></a></li>').hide();
            }

            else var node = $('<li class="ui-state-default ui-element" title="'+option.text()+'"><span class="ui-icon"/>'+option.text()+'<a href="#" class="ui-state-default action"><span class="ui-corner-all ui-icon"/></a></li>').hide();
        }

        // if type == 'storyline-episode', show expired episode in red
        else if (this.options.type == 'storyline-episode') {
            var expired_string_pos = option.text().search(new RegExp(/ - /i));

            if (expired_string_pos >= 0) {
                var option_text_expired = option.text().substr(0, expired_string_pos);               
                var option_text_expired_css = '<div style="color: #ED1C24;">' + option.text() + '</div>';
                var node = $('<li class="ui-state-default ui-element" title="' + option.text() + '"><span class="ui-icon"/>' + option_text_expired_css + '<span class="ui-icon">Expired</span></li>').hide();
            }
            ...
            var node = $('<li class="ui-state-default ui-element" title="'+option.text()+'"><span class="ui-icon"/>'+option.text()+'<a href="#" class="ui-state-default action"><span class="ui-corner-all ui-icon"/></a></li>').hide();
        }
	...
	_batchSelect: function(elements, state) {
		...
		setTimeout(function() {
			...
			elements.each(function(i,element) {
                if (that.options.type == 'storyline-episode') {
                    var expired_string_pos = $(element).text().search(new RegExp(/ - Expired/i));
                    if (expired_string_pos <= 0) that._setSelected($(element), state);
                }
                else that._setSelected($(element), state);
			});
   ...
   // @param bool noclone (optional) true only if item should not be cloned on the other list
	...
	_setBusy: function(state) {
		...
		// DEBUG
    //console.log("Busy state changed to: %s (%s)", state, this.busy);
	},
  ...
  _filterSelected: function(elements) { 
		var input = this.selectedActions.children('input.search-selected');
		var term = $.trim( input.val().toLowerCase() );
	...
	_registerSearchEvents: function(input, searchNow) {
        ...
        if (that.options.remoteUrl && $(input).hasClass('search') ) { /* if (that.options.remoteUrl ) {*/
          ...
          if($(input).hasClass('search-selected')){
            that._filterSelected(that.selectedList.children('li.ui-element'));
          }
          else{
...
var _dragHelper = function(event, ui) {
  ...
  try{  
    ...
    ;
    return clone;
  } catch(e) {console.log('_dragHelper: %s', e.description);}
...
$.extend($.ui.multiselect, {
    ...
    locale: {
        ...
        itemsCount:'#{count} item(s) selected',
