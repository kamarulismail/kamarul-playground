...
class YiiMail extends CApplicationComponent
{
  ...
  {    
    ...
    $port = isset($this->transportOptions['port']) ? $this->transportOptions['port'] : 80;
    ...
    try{
      ...
      $message = Yii::t('yii','Exception: {exceptionMsg}.', array('{exceptionMsg}' => $e->getMessage()));          
      Yii::log($message, CLogger::LEVEL_ERROR, 'ext.yii-mail.YiiMail');
    ...
    if(!$fp)
    {
      $message = Yii::t('yii','Failed to connect to host "{host}". {errorStr}', array('{host}' => $host, '{errorStr}' => $errstr));          
      Yii::log($message, CLogger::LEVEL_ERROR, 'ext.yii-mail.YiiMail');
